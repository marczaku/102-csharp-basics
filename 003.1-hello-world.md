...
- Open your Command Line. On Mac/Linux: `Terminal` on Windows: `Command Prompt` or short: `cmd.exe`
- First, use `cd path/to/directory` to move to the desired folder (replace `path/to/directory` with whereever you want to place your projects)
  - e.g.: `cd C:\Projects\` (Windows) or `cd ~/Projects/` (Mac/Linux)
  - if that directory does not exist, yet...
  - either use `mkdir path/to/directory` to create said directory through the Command Line (replace `path/to/directory` again)
  - or create it manually through `Finder` (Mac) or `File Explorer` (Windows)
