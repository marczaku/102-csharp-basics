...
- `System.Math` is a `static` class containing many useful functions for numbers
  - `static` means, it can not be instantiated, you can not write `Maths maths = new Maths();`
...
```
- `double Abs(double)` returns the absolute of a number, which is always positive. Useful for calculatung distances.
```cs
...
```
- `double Round(double)` returns the value of the number rounded to the lower integer
```cs
double floor = Math.Floor(12.6); // 12 is the next lower integer
```
- `double Round(double)` returns the value of the number rounded to the higher integer
```cs
...
```
- We can combine strings into a larger strings using the `+`-Operator
```cs
...
```cs
string char1 = firstName[0]; // 'J'
```
- You can also get the index of the first occurance of a character or string. Again, counting starts at `0`
```cs
string index = firstName.IndexOf("h"); // 2
```
- You can get a sub-part of a tring starting at a certain index (1) with a certain length (2)
```cs
string sub = firstName.SubString(1, 2); // "hn"
```
- You can replace all occurances of a character or string with another one
```cs
string replace = concat.Replace( 'n', 'd'); // "Johd Kade"
```
...
- You can inverse them by using an exclamation mark `!`
```cs
...
- The ternary operator can be used as a short-cut for `if`-`else`-blocks, when assigning a value:
```cs
...
- Initializing a new Random Instance with a Random Seed
```cs
Random random = new Random();
```

- Initializing a new Random Instance with a Custom Seed
```cs
Random random = new Random(23746);
```

- Receive a new randomNumber between `0` (inclusive) and `5` (exclusive), so either `0`, `1`, `2`, `3` or `4`
```cs
int number = random.Next(0, 5); 
```

- Get any random integer between `0` and `int.MaxValue` (1.2 bln)
```cs
int number = random.Next();
```

- Receive a new randomNumber between 0 (inclusive) and 1 (exclusive), so anything between 0.000000 and 0.9999999
```cs
int number = random.NextDouble();
```
